        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 w-full items-stretch max-w-full mx-auto relative z-10">
          {plans.map((plan) => (
            <motion.div
              ...
              className={`relative rounded-4xl border flex flex-col shadow-lg h-full cursor-pointer transition-all duration-500 p-8 pb-10 ${
                activePlan === plan.id ? "border-white/20 bg-[#2a2a2a] z-20 shadow-2xl" : "border-white/5 bg-[#141414] z-10"
              }`}
            >
              <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-4xl">
                <div
                  className={`absolute -bottom-10 right-0 w-[200px] md:w-[240px] aspect-square pointer-events-none transition-all duration-500 ${activePlan === plan.id ? "opacity-80 scale-110" : "opacity-40"}`}
                >
                  <img src={plan.icon} alt={`${plan.name} Decor`} className="absolute inset-0 w-full h-full object-contain" />
                </div>
              </div>
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <h3 className="font-serif text-[28px] md:text-[36px] tracking-wide text-white font-fraunces mb-2">{plan.name}</h3>

                {plan.duration ? <div className="text-white/60 text-[14px] md:text-[16px] mb-6">{plan.duration}</div> : <div className="h-[21px] mb-6"></div>}
              ...
              {activePlan === plan.id && (
                <motion.div layoutId="exploreButton" className="absolute -bottom-4 left-1/2 -translate-x-1/2 z-30">
                  <button
                    onClick={() => handleAuthRedirect("/login")}
                    className="px-6 cursor-pointer py-2.5 bg-white text-black text-[12px] font-bold rounded-full shadow-[0_10px_20px_rgba(0,0,0,0.3)] hover:bg-gray-200 transition-colors whitespace-nowrap"
                  >
                    Explore Membership
                  </button>
                </motion.div>
