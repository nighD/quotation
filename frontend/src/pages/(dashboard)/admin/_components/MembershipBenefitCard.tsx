...
const formatCardNumber = (num?: string): string => {
  ...
  return noSpaces.match(/.{1,4}/g)?.join(" ") || num;
};

export const MembershipBenefitCard: React.FC<MembershipBenefitCardProps> = ({ onAddCard }) => {
  const { user } = useAuth();
  const hasCard = Boolean(user?.card_number && user.card_number.trim() !== "");
    ...
    <>
      <div className="w-full bg-[#E8D7C9] rounded-3xl sm:rounded-[28px] p-3.5 sm:p-5 shadow-sm flex flex-col justify-between border border-[#dfd3c7]">
        <motion.div
          ...
          onClick={!hasCard ? handleAddCard : undefined}
        ...
        <div>
          <div className="flex items-center justify-between mb-2 sm:mb-2.5 select-none">
            <h3 className="text-[#1B1A16] font-['Inter'] font-semibold! text-[13px] sm:text-[15px]">Benefit Slot</h3>
          </div>

          <div className="bg-white rounded-2xl p-2.5 sm:p-3.5 shadow-xs grid grid-cols-4 gap-1.5 sm:gap-2.5 border border-[#f0e8e0]">
            {[
              { id: "event", title: "Sự kiện", fullTitle: "Sự kiện", active: 4, total: 8 },
              { id: "course", title: "Khóa học chuyên sâu", fullTitle: "Khóa học chuyên sâu", active: 4, total: 8 },
              { id: "report", title: "Report", fullTitle: "Report", active: 1, total: 8 },
              { id: "newsletter", title: "Newsletter", fullTitle: "Newsletter", active: 1, total: 8 },
            ].map((slot) => (
              <div key={slot.id} className="flex flex-col gap-1 min-w-0">
                <span
                  className="font-['Cormorant_Garamond'] text-[#1B1A16] text-[11.5px] xs:text-[12.5px] sm:text-[13.5px] md:text-[14.5px] font-semibold truncate"
                  title={slot.fullTitle || slot.title}
                >
                  {slot.title}
                </span>
                <div className="flex items-center gap-0.5 sm:gap-1 flex-wrap">
                  {hasCard ? (
                    <>
                      {[...Array(slot.active)].map((_, i) => (
                        <span key={`${slot.id}-active-${i}`} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#D16419]" />
                      ))}
                      {[...Array(slot.total - slot.active)].map((_, i) => (
                        <span key={`${slot.id}-inactive-${i}`} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#E8D7C9]" />
                      ))}
                    </>
                  ) : (
                    [...Array(slot.total)].map((_, i) => <span key={`${slot.id}-skel-${i}`} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#E8D7C9]" />)
                  )}
                </div>
              </div>
            ))}
