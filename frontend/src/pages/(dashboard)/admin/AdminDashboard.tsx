...
export function AdminDashboard() {
  ...
  const [submittingBookingType, setSubmittingBookingType] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [statsResponse, bookingResponse, eventResponse] =
          await Promise.all([
        ...
        if (
          ...
          const bookingTypes = bookingResponse.data.data
            ...
            .filter((bookingType: string | undefined): bookingType is string =>
              Boolean(bookingType),
            );
    ...
    try {
      ...
      if (data.success) {
        setRequestedBookingTypes((prev) =>
          prev.includes(request.bookingType)
            ? prev
            : [...prev, request.bookingType],
        );
      }
    } catch (_error: any) {
    } finally {
    ...
    <div className="w-full space-y-4 sm:space-y-6">
      ...
      <header className="px-1 py-1 sm:py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
        <h1 className="font-['Cormorant_Garamond']! tracking-tight">
          <span className="italic text-[20px] sm:text-[22px] md:text-[24px] font-semibold! text-[#B58F6F] opacity-90">
            Xin chào,
          </span>{" "}
      ...
      {/* Row 1: Membership Benefit Card + Booking Section */}
      ...
      {/* Row 2: Report Section + (Event Section & Private Club) */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 sm:gap-6 items-stretch">
        <div className="xl:col-span-5 2xl:col-span-5 flex">
          <ReportSection />
        </div>
