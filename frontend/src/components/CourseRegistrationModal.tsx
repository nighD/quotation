...
import { X, CheckCircle2, BookOpen, Clock, User, Sparkles } from "lucide-react";
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4 mb-4 sm:mb-5">
              <div className="flex-1 pr-2">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#EADCCF] text-[#8C6246] text-[10px] sm:text-[11px] font-['Inter']! font-semibold uppercase tracking-wider mb-2">
                  <Sparkles size={12} className="text-[#B58F6F]" />
                  <span>{course?.badge || "Chi tiết khóa học"}</span>
                </div>
                <h2 className="text-[24px] sm:text-[28px] font-['Cormorant_Garamond']! font-semibold! text-[#1B1A16] leading-tight">{activeTitle}</h2>
                <p className="mt-1.5 text-[12px] sm:text-[13px] font-['Inter']! text-[#664E48] leading-relaxed">{activeDescription}</p>
              </div>

              <button
                type="button"
                onClick={onClose}
                className="w-8 h-8 rounded-full bg-stone-200/60 text-[#664E48] hover:text-[#1B1A16] hover:bg-stone-300/60 flex items-center justify-center cursor-pointer transition-colors shrink-0"
              >
                <X size={16} />
              </button>
            </div>
              ...
              <div className="py-6 sm:py-8 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[12px] sm:text-[13px] font-['Inter']! text-[#664E48] max-w-sm mx-auto leading-relaxed">
                    Hệ thống đã tiếp nhận thông tin đăng ký khóa học của bạn. Ban tổ chức sẽ liên hệ hỗ trợ trong thời gian sớm nhất.
                  </p>
                </div>
                  ...
                  <>
                    ...
                    <label className="block">
                      ...
                      <textarea
                        ...
                        className="w-full min-h-[96px] sm:min-h-[110px] rounded-xl sm:rounded-2xl border border-[#D9C8BA] bg-white px-3.5 py-2.5 sm:py-3 text-[13px] text-[#1B1A16] outline-none focus:border-[#B58F6F] transition-colors resize-none leading-relaxed"
                        placeholder="Nội dung cần hỗ trợ hoặc câu hỏi cho giảng viên..."
                      />
                    </label>
                  </>
                ) : null}
