...
const UpgradeModalContent: React.FC<UpgradeModalProps> = ({ isOpen, onClose, onRequestUpdated, onSuccess }) => {
    ...
    upgradeRequest && upgradeRequest.status !== "rejected" ? (
      <div className="w-full flex justify-end">
        <button
          type="button"
          onClick={onClose}
          className="bg-[#523C37] hover:bg-[#382b24] text-white text-[12px] font-['Inter']! font-medium px-5 py-2.5 rounded-xl uppercase tracking-wider cursor-pointer"
        >
          Close
        </button>
      </div>
  ...
  return (
    <PortalModal
      ...
      description="Gửi thông tin cho admin để xét duyệt, cấp số thứ tự và gắn role cho tài khoản."
    ...
    >
      <div className="space-y-4">
        {upgradeMessage && (
          <div
            className={`rounded-2xl px-4 py-3 text-[12px] font-['Inter']! ${
